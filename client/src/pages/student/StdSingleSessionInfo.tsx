...
const SingleSessionInfo = () => {
    ...
    useEffect(() => {
        async function isSessionInWishlist() {
          try {
            const response = await axiosInstance.get(`/student/wishlist/check/${id}`);
            console.log("response.data in wislist check", response.data.wishlist);
            const {isInWislist} = response.data.wishlist;
            console.log("is in wisshhh", isInWislist);
            
            
            setWishlistStatus(isInWislist);
            setLoading(false);
          } catch (error) {
    ...
    const patchWishlist = async () => {
        try {
            const response = await axiosInstance.patch(`/student/wishlist`, {sessionId: id});
                        ...
                        <div className='ml-44 bg-white w-3/12 h-[535px] flex flex-col justify-center items-center border-2 border-gray-300'>
                            ...
                            <div className='p-5 bg-white w-full h-1/2 rounded-bl-md rounded-br-md'>
                                <div className='flex items-center space-x-3'>
                                    <img src={session?.instructorId?.image?.url} className=' w-12 h-12 object-cover rounded-full' alt="" />    
